...
cat("Independence not violated. We check the normality through plot and Shapiro test \n")
  ...
  #Homoscedacity----------
     cat("We are checking the homogeneity of variance for each group. Levene's Test \n")
...
# Define the function to copy a file to a specified folder
# copy_file_to_folder <- function(new_folder, overwrite = TRUE) {
#   # Get the current R script file path (works in RStudio)
...
#   new_file_path <- file.path(new_folder, basename(current_file))
#   
#   # Copy the file to the new folder, overwriting if necessary
#   success <- file.copy(current_file, new_file_path, overwrite = overwrite)
#   
#   # Return success status
#   if (success) {
#     message("File copied successfully to ", new_file_path)
#   } else {
...
# 
# Example usage:
# Call the function with the new folder and overwrite option
#copy_file_to_folder("C:/Users/garog/Documents/GitHub/Statistical-Testing-in-R", overwrite = TRUE)
